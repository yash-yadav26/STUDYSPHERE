const PaymentStats = ({ payments }) => {
  ...
  const totalRevenue = payments.reduce(
    (sum, payment) =>
      payment.paymentStatus === "Paid" ? sum + payment.amount : sum,
    0,
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div className="bg-white p-5 rounded-xl shadow">
        ...
        <p className="text-2xl font-bold text-green-600 mt-2">{paidPayments}</p>
      </div>

      <div className="bg-white p-5 rounded-xl shadow">
        <h3 className="text-gray-500 text-sm">Pending / Failed</h3>
